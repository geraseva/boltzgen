...
class NASelector(Selector):
    """Select design tokens from NA chains."""
    ...
    def __init__(
        ...
        self.selection_functions = {
            ...
            "select_nonna_interface": self.select_nonna_interface,
            "select_standard_na": self.select_standard_na,
            "select_na_intefaces": self.select_na_intefaces,
            "select_na_chains": self.select_na_chains,
        }
        self.probabilities = {
            "select_all": [
                (0, "select_none"),
                ...
                (0, "select_nonna_interface"),
                (1, "select_standard_na"),
                (0, "select_na_intefaces"),
                (0, "select_na_chains"),
            ],
            "0na_>=0nonna": [
                (1, "select_none"),
            ...
            "1na_>0nonna": [
                ...
                (0, "select_scaffold_binder"),
                (0, "select_motif_binder"),
                (0.2, "select_nonna_interface"),
                (0.4, "select_standard_na"),
                (0, "select_na_intefaces"),
                (0, "select_na_chains"),
            ],
            ">1na_0nonna": [
                ...
                (0, "select_nonna_interface"),
                (0.1, "select_standard_na"),
                (0.1, "select_na_intefaces"),
                (0.4, "select_na_chains"),
            ],
            ">1na_>0nonna": [
                (0.05, "select_none"),
                (0.2, "select_scaffold"),
    ...
    def select(  # noqa: PLR0915
        ...
        """Select NA residues to be designed.
        ...
        # Get chains
        na_chain_ids = np.unique(na_tokens["asym_id"])
        num_na_chains = len(na_chain_ids)
        nonna_chain_ids = np.unique(nonna_tokens["asym_id"])
        num_nonna_chains = len(nonna_chain_ids)

        # Get selection distribution
        if self.select_all:
            task = "select_all"
        elif num_na_chains == 0:
            task = "0na_>=0nonna"
        elif num_na_chains == 1 and num_nonna_chains == 0:
            task = "1na_0nonna"
        elif num_na_chains == 1 and num_nonna_chains > 0:
            task = "1na_>0nonna"
        elif num_na_chains > 1 and num_nonna_chains == 0:
    ...
    def select_motif(
        ...
        if fixed_crop:
            num_crop = min(len(na_tokens) // 4, 20)
        else:
            num_crop = max(random.integers(len(na_tokens)), neighborhood_size + 1)

        query = na_tokens[random.integers(len(na_tokens))]
        crop_indices = self.cropper.select_cropped_indices(
            tokens=na_tokens,
    ...
    def select_scaffold(
        ...
        na_tok_mask = np.ones(len(na_tokens))
        na_tok_mask[crop_indices] = 0

        design_indices = na_tokens["token_idx"][na_tok_mask.astype(bool)]
    ...
    def select_standard_na(self, tokens, random):
        na_mask = (tokens["mol_type"] == const.chain_type_ids["DNA"]) | (tokens["mol_type"] == const.chain_type_ids["RNA"]) 
        standard_mask = tokens["is_standard"].astype(bool)
        tokens["design_mask"][na_mask & standard_mask] = True
        return tokens

    def select_nonna_interface(self, tokens, random):
        na_mask = (tokens["mol_type"] == const.chain_type_ids["DNA"]) | (tokens["mol_type"] == const.chain_type_ids["RNA"]) 
        standard_mask = tokens["is_standard"].astype(bool)
        na_tokens = tokens[na_mask & standard_mask]
        ...
        nonna_chain_ids = np.unique(nonna_tokens["asym_id"])
        num_target_chains = random.choice(np.arange(1, len(nonna_chain_ids) + 1))
        select_ids = random.choice(nonna_chain_ids, num_target_chains, replace=False)
        ...
        noisy_distances = min_token_distances(
            na_tokens, target_tokens, random, self.distance_noise_std
        )
        indices = np.argsort(noisy_distances)
        num_selected = random.choice(np.arange(1, len(na_tokens) + 1))
        selected_tokens = na_tokens[indices[:num_selected]]
        tokens["design_mask"][selected_tokens["token_idx"]] = True
        return tokens

    def select_na_chains(self, tokens, random):
        na_mask = (tokens["mol_type"] == const.chain_type_ids["DNA"]) | (tokens["mol_type"] == const.chain_type_ids["RNA"]) 
        standard_mask = tokens["is_standard"].astype(bool)
        na_tokens = tokens[na_mask & standard_mask]

        na_chain_ids = np.unique(na_tokens["asym_id"])
        assert len(na_chain_ids) > 1
        num_selections = random.choice(np.arange(1, len(na_chain_ids)))
        select_ids = random.choice(na_chain_ids, num_selections, replace=False)

        for id in select_ids:
            tokens["design_mask"][(id == tokens["asym_id"]) & standard_mask] = True
        return tokens

    def select_na_intefaces(self, tokens, random):
        na_mask = (tokens["mol_type"] == const.chain_type_ids["DNA"]) | (tokens["mol_type"] == const.chain_type_ids["RNA"]) 
        standard_mask = tokens["is_standard"].astype(bool)
        na_tokens = tokens[na_mask & standard_mask]
